...
def lasso_vec_cd(
    ...
    Beta[:, k-1] =  _classo_cd(XTX, XTy, X_norm2_inv, np.zeros(p, dtype=np.float64), lams[k-1], abs_tol, iter_max)
    for i in range(k - 2, -1, -1):
        Beta[:, i] = _classo_cd(XTX, XTy, X_norm2_inv, Beta[:, i+1], lams[i], abs_tol, iter_max)
