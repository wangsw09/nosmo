...
cdef np.ndarray[dtype=np.float64_t, ndim=1] _cridge_cd(
    ...
    while iter_diff > abs_tol:
        ...
        for i in range(p):
            betai0 = beta[i]
            beta[i] = grad_move[i] / (1.0 + 2.0 * lam * X_norm2_inv[i])
...
def ridge_cd(
    ...
    return _cridge_cd(XTX, XTy, X_norm2_inv, np.zeros(p, dtype=np.float64), lam, q, abs_tol, iter_max)
