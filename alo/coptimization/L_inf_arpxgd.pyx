...
from ..clinalg.cython_blas_wrapper cimport st_dgemv, sn_dgemv, s_daxpy, mat_T_mat, s_dcopy, s_dnrm2, s_ddot
from ..cprox.l_inf_prox cimport L_inf_prox

@cython.boundscheck(False)
@cython.wraparound(False)
cdef np.ndarray[dtype=np.float64_t, ndim=1] _l_inf_arpxgd(
...
def L_inf_arpxgd(
    ...
    return _l_inf_arpxgd(X, y, beta_init, lam, ss, abs_tol, iter_max)

def L_inf_vec_arpxgd(
        np.ndarray[dtype=np.float64_t, ndim=2] X,
        np.ndarray[dtype=np.float64_t, ndim=1] y,
        np.ndarray[dtype=np.float64_t, ndim=1] beta_init,
    ...
    Beta[:, k-1] = _l_inf_arpxgd(X, y, beta_init, lams[k-1], ss, abs_tol, iter_max)
    for i in range(k - 2, -1, -1):
        Beta[:, i] = _l_inf_arpxgd(X, y, Beta[:, i + 1].copy(), lams[i], ss, abs_tol, iter_max)
    return Beta
